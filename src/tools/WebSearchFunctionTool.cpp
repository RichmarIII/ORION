...
WebSearchFunctionTool::Execute(Orion& Orion, const web::json::value& Parameters)
{
    ...
    web::http::client::http_client SearchClient(U("https://www.googleapis.com"));
    web::http::uri_builder         SearchURIBuilder(U("customsearch/v1"));
    SearchURIBuilder.append_query(U("key"), U(Orion.GetGoogleAPIKey()));
    SearchURIBuilder.append_query(U("q"), Parameters.at(U("query")).as_string());
    SearchURIBuilder.append_query(U("cx"), U(Orion.GetGoogleCustomSearchEngineID()));

    // Send the request
    web::http::http_request SearchRequest(web::http::methods::GET);
    SearchRequest.set_request_uri(SearchURIBuilder.to_string());
    const auto SEARCH_RESPONSE = SearchClient.request(SearchRequest).get();

    if (SEARCH_RESPONSE.status_code() != web::http::status_codes::OK)
    {
        auto ErrorObject                                       = web::json::value::object();
        ErrorObject[FunctionResultStatics::NAME_RESULT.data()] = web::json::value::string(U("Failed to search the web: ") + SEARCH_RESPONSE.reason_phrase());
        return ErrorObject.serialize();
    }

    // Parse the response
    const auto JSON_RESPONSE_BODY = SEARCH_RESPONSE.extract_json().get();

    // Create the result
    auto SearchResult = web::json::value::object();
    SearchResult[FunctionResultStatics::NAME_ORION_INSTRUCTIONS.data()] =
        web::json::value::string(U("The navigate_link function should be used on any returned links to complete the search until the answer is found. "
    ...
    auto& ItemsArray = SearchResultResultRef[U("items")] = web::json::value::array();

    for (auto& Item : JSON_RESPONSE_BODY.at(U("items")).as_array())
    {
        auto ItemObject = web::json::value::object();
